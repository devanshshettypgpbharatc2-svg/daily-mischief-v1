...
function QtyControl({ item }: { item: CartItem }) {
  ...
  return (
    <div className="flex items-center mt-3" role="group" aria-label="Quantity">
      <button
        onClick={() => item.quantity === 1 ? removeItem(item.lineId) : updateQuantity(item.lineId, item.quantity - 1)}
        disabled={loading}
      ...
      <div className={cn(
        'w-10 md:w-9 h-11 md:h-7 flex items-center justify-center',
...
function EmptyCart({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full py-16 px-6 text-center">
      <div className="w-14 h-14 border border-white/[0.08] flex items-center justify-center mb-7" aria-hidden="true">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1">
      ...
      <p className="font-serif italic text-[20px] font-light text-white/45 leading-[1.3] mb-8">
        Looks like you're<br />behaving today.
      </p>
      <button
        onClick={onClose}
      ...
      >
        ...
        <svg className="relative z-10" width="12" height="8" viewBox="0 0 12 8" fill="none" stroke="currentColor" strokeWidth="1" aria-hidden="true">
        ...
        <>
          ...
          >
            ...
            <div className="flex items-start justify-between px-7 pt-6 pb-5 border-b border-white/[0.07] flex-shrink-0">
              <div>
                ...
                <p className="font-sans text-[9px] tracking-[0.35em] uppercase text-white/30 mt-1">
                  {qty === 0 ? 'Nothing here yet' : `${qty} piece${qty !== 1 ? 's' : ''} of mischief`}
                </p>
              </div>
              <button
                ref={closeRef}
            ...
            <div className="flex-1 overflow-y-auto px-7 [&::-webkit-scrollbar]:w-px [&::-webkit-scrollbar-thumb]:bg-white/10">
              {isLoading ? (
                <div><CartItemSkeleton /><CartItemSkeleton /></div>
              ) : !cart || cart.items.length === 0 ? (
                <EmptyCart onClose={closeCart} />
              ) : (
                <AnimatePresence mode="popLayout">
                  {cart.items.map(item => <CartLineItem key={item.lineId} item={item} />)}
                </AnimatePresence>
              )}
            </div>
              ...
              {cart && cart.items.length > 0 && (
                ...
                >
                  <div className="flex items-baseline justify-between mb-1.5">
                    <span className="font-sans text-[9px] tracking-[0.3em] uppercase text-white/35">Subtotal</span>
                    <span className="font-serif text-[24px] font-light">{formatMoney(cart.subtotal)}</span>
                  </div>
                  ...
                  >
                    ...
                    <svg className="relative z-10 transition-transform duration-400 group-hover:translate-x-[5px]" width="14" height="10" viewBox="0 0 14 10" fill="none" stroke="currentColor" strokeWidth="1" aria-hidden="true">
